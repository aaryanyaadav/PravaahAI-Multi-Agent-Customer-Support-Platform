...
def handle_fallback(query: str, session_id: str, account_id: Optional[str], error_message: str) -> Dict[str, Any]:
    ...
    """
    q_lower = query.lower()
    guard_res = validate_input(query)
    
    if guard_res.get("is_conversational") and guard_res.get("direct_response"):
        response_text = guard_res.get("direct_response")
        is_support = False
    elif not guard_res["safe"]:
        response_text = f"Request blocked: {guard_res['reason']}"
        is_support = False
    else:
        ...
        if any(k in q_lower for k in support_keywords):
            is_support = True
        elif len(query) > 20 and not re.match(r"^(hello|hi|hey|greetings|good morning|good afternoon|good evening|test|asdf|qwerty|thanks|okay)\b", q_lower):
            is_support = True

    resolved_account_id = account_id or get_fallback_account_id()
    ticket_id = None

    if not is_support and not guard_res.get("is_conversational") and guard_res["safe"]:
        response_text = (
...
async def get_dashboard_stats(days: Optional[int] = None):
    ...
    try:
        ...
        # Query tickets and audit_log from Supabase with graceful fallback
        try:
            from database.connections import supabase
            if supabase:
            ...
            print(f"Supabase telemetry query unavailable ({db_err}). Falling back to local data.")
            if os.path.exists("tickets.csv"):
                try:
                    ...
                    with open("tickets.csv", "r", encoding="utf-8") as f:
                        ...
                        for r in reader:
                            tickets_data.append(dict(r))
                except Exception as csv_err:
                    print(f"Error loading local tickets.csv: {csv_err}")
